...
namespace AudioShell
{
    ...
    /// </summary>
    /// <remarks>
    /// Consumers of the AudioShell library typically create new <see cref="TaggedAudioFile"/> objects directly. During
    ...
    public class TaggedAudioFile : AudioFile
    {
        ...
        public void SaveMetadata(SettingsDictionary settings = null)
        {
            ...
            using (FileStream fileStream = FileInfo.Open(FileMode.Open, FileAccess.ReadWrite))
            {
                ...
                foreach (var encoderFactory in ExtensionProvider<IMetadataEncoder>.Instance.Factories.Where(factory => string.Compare((string)factory.Metadata["Extension"], FileInfo.Extension, StringComparison.OrdinalIgnoreCase) == 0))
                {
        ...
        void LoadMetadata(Stream stream)
        {
            ...
            foreach (var decoderFactory in ExtensionProvider<IMetadataDecoder>.Instance.Factories.Where(factory => string.Compare((string)factory.Metadata["Extension"], FileInfo.Extension, StringComparison.OrdinalIgnoreCase) == 0))
            {
                try
                {
